...
$sql2 = "SELECT * FROM `users` WHERE `id` = $new_aan";

$result2 = $conn2->query($sql2);
if ($result2->num_rows > 0) {
    // output data of each row
    while($row = $result2->fetch_assoc()) {
        $new_usermail = $row["username"];
    }
} else {
    echo '<script>swal.fire("Er ging iets mis...", "We koneden de data van de ontvanger niet ophalen!.", "error");</script>';
    die();
}
$conn2->close();
...
if ($conn->query($sql) === TRUE) {
    ...
    if ($audit_conn->query($audit_sql) === TRUE) { $audit_conn->close(); } else { echo "Error: " . $sql . "<br>" . $conn->error; $audit_conn->close(); }
    //Audit log end
