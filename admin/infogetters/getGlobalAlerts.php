...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<script>function resendNumer" . $row['id'] . "() { ws.send(JSON.stringify({ id: 'send-admin-alert', titel: '" . $row["mes_title"] . "', text: '" . $row["mes_text"] . "' })); ws.send(JSON.stringify({ id: 'audit-log', user: " . $gebid . ", actie: 'Admin Systeem Notifactie verstuurd', info: 'Verstuurde notificatie met titel " . $row["mes_title"] . "' }));}</script>";
        echo '</tr>';
    }
} else {
    echo "<tr>";
